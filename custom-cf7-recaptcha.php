...
class Custom_CF7_ReCaptcha {
    ...
    public function inject_recaptcha_field( $content ) {
        ...
        if ( 'v3' === $version ) {
            ...
            $content = str_replace(
                '</form>',
                '<div class="g-recaptcha" data-sitekey="' . esc_attr( $this->options['v2_site_key'] ) . '" data-theme="' . esc_attr( $theme ) . '"></div></form>',
                $content
            );
        }
        return $content;
    ...
    public function verify_recaptcha( $spam, $submission ) {
        ...
        if ( 'v3' === $version && isset( $result->score ) && $result->score < 0.5 ) {
            return true;
        }
    ...
    public function settings_init() {
        ...
        add_settings_section(
            'custom_cf7_recaptcha_section',
            __( 'Google reCAPTCHA Settings', 'custom-cf7-recaptcha' ),
            null,
            'customCf7Recaptcha'
        );
        ...
        add_settings_field(
            'recaptcha_version',
            __( 'reCAPTCHA Version', 'custom-cf7-recaptcha' ),
            array( $this, 'render_field' ),
            'customCf7Recaptcha',
            'custom_cf7_recaptcha_section',
            ['type' => 'select', 'id' => 'recaptcha_version', 'options' => ['' => '-- Select Version --', 'v2' => 'Version 2 ("I\'m not a robot" Checkbox)', 'v3' => 'Version 3 (Invisible Score-Based)']]
        );
